...
#define LIGNES 6		// ligne du jeu
#define COLONNES 7		// colonne du jeu
...
int jouerCoup( Etat * etat, Coup * coup ) {
	...
	/* par exemple : */
	if(coup->colonne<0 || coup->colonne > COLONNES -1){
		return 0;
	}else{

		int ligne = 0;
		int stop = 0;
		if(etat->plateau[ligne][coup->colonne] != -1){
			return 0;
		}
		while(etat->plateau[ligne+1][coup->colonne] == -1){
			ligne ++;
		}
		...
		etat->plateau[ligne][coup->colonne] = etat->joueur;
...
Coup ** coups_possibles( Etat * etat ) {
	...
	for (j=0; j < COLONNES; j++) {
		Coup* c = nouveauCoup(j);
		if(jouerCoup(etat,c ) != 0){
			coups[k] = c; 
			k++;
		}				
	...
	for ( i=0;i < LIGNES; i++) {
		for(j=0; j < COLONNES; j++) {
			if ( etat->plateau[i][j] != -1) {
				...
				// lignes
				k=0;
				while ( k < 3 && i+k < 4 && etat->plateau[i+k][j] == etat->plateau[i][j] ) 
					k++;
				if ( k == 4 ) 
