...
public class StateC4 extends mcts_uct.State{
        ...
        for ( i=0;i < plate.length; i++) {
            for(j=0; j < plate[0].length; j++) {
                ...
                if ( this.plate[i][j] != -1) {
                    ...
                    // plate.length
                    k=0;
                    while ( k < 4 && j+k < plate[0].length && this.plate[i][j+k] == this.plate[i][j] )
                        k++;
                    if ( k == 4 ) {
                        return this.plate[i][j] == 1 ? End.COMPUTER_WIN : End.HUMAN_WIN;
                    }


                    // plate[0].length
                    k=0;
                    while ( k < 4 && i+k < plate.length && this.plate[i+k][j] == this.plate[i][j] )
                        k++;
                    if ( k == 4 ) {
                        return this.plate[i][j] == 1 ? End.COMPUTER_WIN : End.HUMAN_WIN;
                    }
                    ...
                    // diagonales
                    k=0;
                    while ( k < 4 && i+k < plate.length && j+k < plate[0].length && this.plate[i+k][j+k] == this.plate[i][j] )
                        k++;
                    if ( k == 4 ){
                        return this.plate[i][j] == 1 ? End.COMPUTER_WIN : End.HUMAN_WIN;
                    }
                    ...
                    k=0;
                    while ( k < 4 && i+k < plate.length && j-k >= 0 && this.plate[i+k][j-k] == this.plate[i][j] )
                        k++;
                    if ( k == 4 ){
                        return this.plate[i][j] == 1 ? End.COMPUTER_WIN : End.HUMAN_WIN;
                    }
    ...
    public boolean isValideAction(Action action) {
        ...
        return (
                ((ActionC4)(action)).getColumn() >= 0 &&
                ((ActionC4)(action)).getColumn() < this.plate[0].length &&
                 this.plate[0][((ActionC4)(action)).getColumn()] == -1
        );
    }
